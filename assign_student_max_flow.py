...
def main():
    ...
    for i in range(len(student_nodes)):
        smcf.add_arc_with_capacity_and_unit_cost(0, student_nodes[i], 2, 0)

    # add edges from student nodes to event nodes
    event_nodes = list(range(node_count, events * 2 + node_count))
    ...
    for event_node in event_nodes:
        smcf.add_arc_with_capacity_and_unit_cost(event_node, sink_node, 2, 0)

    # add flow volume to source and sink nodes
    smcf.set_node_supply(source_node, events * 2)
    smcf.set_node_supply(sink_node, -events * 2)
